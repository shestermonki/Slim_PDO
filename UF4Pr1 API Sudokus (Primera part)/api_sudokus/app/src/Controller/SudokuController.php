...
class SudokuController
{
    ...
    {
        $sudokus = \ApiSudoku\Model\SudokuDB::getInstance()->getAllSudokus();
        if (is_null($sudokus)) {
    ...
    {
        $id = $args['id'];
        $sudoku = \ApiSudoku\Model\SudokuDB::getInstance()->getSudokuById($id);
        if (is_null($sudoku)) {
    ...
    {
        $id = $args['id'];
        $player = \ApiSudoku\Model\SudokuDB::getInstance()->getPlayerById($id);
        if (is_null($player)) {
            $response = $response->withStatus(404);
        } else {
    ...
    private function validatePersist($data, $response, $id)
    {
        ...
        */
        ...
        $player = \ApiSudoku\Model\SudokuDB::getInstance()->getPlayerByUsername($username);
        ...
        $player = \ApiSudoku\Model\SudokuDB::getInstance()->persistPlayer($player);
        if (is_null($player)) {
            $response = $response->withStatus(500);
        } else {
            $response = $response->withJson($player);
